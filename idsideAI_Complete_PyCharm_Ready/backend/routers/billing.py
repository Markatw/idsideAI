    ...
    return [
        {
            "id": "free",
            "name": "Free",
            "price_id": os.getenv("STRIPE_PRICE_FREE", "price_free"),
            "amount": 0,
        },
        {
            "id": "starter",
            "name": "Starter",
            "price_id": os.getenv("STRIPE_PRICE_STARTER", "price_starter"),
            "amount": 1900,
        },
        {
            "id": "pro",
...
async def checkout(request: Request):
    ...
    if DRY_RUN:
        with _counter_lock:
            if _counter >= DRY_CAP:
                raise HTTPException(
                    status_code=429, detail="stripe dry-run cap reached"
                )
            _counter += 1
        return {
            "dry_run": True,
    ...
    success_url = (
        ...
        or body.get("successUrl")
        or "http://localhost:5173?checkout=success"
    )
    cancel_url = (
        body.get("cancel_url")
        or body.get("cancelUrl")
        or "http://localhost:5173?checkout=cancel"
    )
...
async def portal(request: Request):
    ...
    return_url = (
        body.get("return_url") or body.get("returnUrl") or "http://localhost:5173"
    )
    if not stripe.api_key:
...
async def webhook(request: Request):
    ...
    if webhook_secret and sig:
        ...
        try:
            event = stripe.Event.construct_from(
                json.loads(payload or b"{}"), stripe.api_key
            )
        except Exception:
            event = {"type": "unknown"}
    ...
    # TODO: Update tenant plan in Neo4j based on checkout.session.completed or customer.subscription.updated
