...
def subgraph(graph_id: str):
    ...
    if ch:
        for e in edges:
            if e.get("type") in ("considers","CHOSEN") and e.get("source")==ch.get("decision") and e.get("target")==ch.get("option"):
                e["chosen"] = True
                e["type"] = e.get("type")
                break
        else:
            edges.append({"id":"CHOSEN","type":"CHOSEN","source":ch["decision"],"target":ch["option"],"chosen":True})
...
def create_snapshot(graph_id: str, root: str = None, depth: int = 3):
    ...
    if not any(n.get("id")=="EV-99" for n in nodes):
        nodes.append({"id":"EV-99","type":"Evidence","title":"SOC2 Report","x":340,"y":430,"confidence":0.6})
        edges.append({"id":"E5","type":"supported_by","source":"EV-99","target":"OPT-A"})
    # snapshots list
...
def derive_why(sample):
    ...
    chosen = next((e for e in edges if e.get("type","").upper()=="CHOSEN" or e.get("chosen")==True), None)
    choice = next((n for n in nodes if chosen and n["id"]==chosen["target"]), None)
    supports = [e for e in edges if e.get("type") in ("supported_by","SUPPORTED_BY") and e.get("target")== (choice["id"] if choice else None)]
    ...
    return {
        ...
        "citations": [{"id":n["id"], "title":n.get("title","Evidence"), "confidence": n.get("confidence")} for n in evs]
    }
...
def export_why(graph_id: str, decision: str = "", to: str = ""):
    ...
    doc = SimpleDocTemplate(pdf_path, pagesize=A4); elements = []
    elements += [Paragraph("IDECIDE — Why Narrative", styles["Title"]), Spacer(1,10)]
    elements += [Paragraph(why["title"], styles["Heading2"]), Paragraph(why["body"], styles["Normal"]), Spacer(1,8)]
    rows = [["ID","Title","Confidence"]]+[[c["id"], c["title"], str(c.get("confidence",""))] for c in why["citations"]]
    table = Table(rows, colWidths=[60,240,80]); table.setStyle(TableStyle([('GRID',(0,0),(-1,-1),0.5,colors.grey)]))
    ...
    d = Document(); d.add_heading("IDECIDE — Why Narrative", 0)
    d.add_heading(why["title"], 1); d.add_paragraph(why["body"])
...
def export_audit(graph_id: str, frm: str = "v1", to: str = "v2"):
    ...
    with zipfile.ZipFile(fpath, "w") as z:
        z.writestr("manifest.json", json.dumps({"graph_id": graph_id, "from": frm, "to": to}))
        z.writestr("snapshot_from.json", json.dumps({"nodes": SAMPLE["nodes"], "edges": SAMPLE["edges"]}))
        z.writestr("snapshot_to.json", json.dumps({"nodes": SAMPLE["nodes"], "edges": SAMPLE["edges"]}))
...
def export_why(graph_id: str, decision: str = "DEC-1", to: str = "latest"):
    ...
    d.save(docx_path)
    return {"pdf": f"/exports/{os.path.basename(pdf_path)}", "docx": f"/exports/{os.path.basename(docx_path)}"}
...
def export_audit(graph_id: str, frm: str = "v1", to: str = "v2"):
    ...
    with zipfile.ZipFile(zpath, "w") as z:
        z.writestr("manifest.json", json.dumps({"graph_id": graph_id, "from": frm, "to": to}, indent=2))
