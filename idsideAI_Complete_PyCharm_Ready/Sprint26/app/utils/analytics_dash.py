...
def summarize(events: list[dict], limit: int = 1000) -> dict:
    ...
    providers = collections.Counter(
        [e.get("provider", "") for e in events if e.get("provider")]
    )
    feedback = collections.Counter(
        [e.get("feedback_type", "") for e in events if e.get("feedback_type")]
    )
    return {
        "total": total,
...
def seed_dashboard(events: list[dict], limit: int = 1000) -> dict:
    ...
    charts = {
        "bar_providers": [
            {"provider": p, "count": c} for p, c in providers.most_common(10)
        ],
