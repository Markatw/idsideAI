...
def layout(tiles: list[dict], width: int) -> list[dict]:
    ...
    r = c = 0
    for t in tiles:
        out.append({"id": t.get("id"), "row": r, "col": c, "w": 1, "h": 1, "title": t.get("title")})
        c += 1
        if c >= cols: c = 0; r += 1
    return out
...
def validate_tiles(tiles: list[dict]) -> dict:
    ...
    for i, t in enumerate(tiles):
        if not t.get("title"):
            errs.append({"index": i, "field": "title", "message": "Provide a tile title."})
        if not t.get("ariaLabel"):
            errs.append({"index": i, "field": "ariaLabel", "message": "Provide an aria-label."})
