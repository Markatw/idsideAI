...
def compare_providers(scenarios: list[dict], providers: list[str]) -> list[dict]:
    ...
    for prov in providers or []:
        ...
        results.append({"provider": prov, "results": prov_res["results"], "summary": prov_res["summary"]})
    return results
...
def export_comparisons_csv(results: list[dict]) -> str:
    ...
    buf = io.StringIO(); w = csv.DictWriter(buf, fieldnames=["scenario","provider","latency_ms","ok","failure_type"])
