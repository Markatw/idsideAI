...
def export_svg(graph: Dict[str, Any], width: int = 640, height: int = 480) -> str:
    ...
    for e in edges:
        ...
        if s and t:
            out.append(
                f'<line x1="{s[0]}" y1="{s[1]}" x2="{t[0]}" y2="{t[1]}" stroke="black" stroke-width="1"/>'
            )
    for nid, (x, y) in pos.items():
        out.append(
            f'<circle cx="{x}" cy="{y}" r="12" fill="lightgray" stroke="black" stroke-width="1"/>'
        )
        out.append(
            f'<text x="{x}" y="{y+4}" font-size="10" text-anchor="middle" fill="black">{html_mod.escape(nid)}</text>'
        )
    out.append("</svg>")
    return "\n".join(out)
