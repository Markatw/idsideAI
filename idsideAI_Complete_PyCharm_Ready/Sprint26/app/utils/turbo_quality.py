...
FAILURE_TYPES = {
    "timeout": lambda e: e.get("error") == "timeout" or (e.get("latency_ms", 0) > e.get("timeout_ms", 999999)),
    "over_token": lambda e: (e.get("prompt_tokens", 0) + e.get("completion_tokens", 0)) > e.get("max_tokens", 999999),
    "refusal": lambda e: isinstance(e.get("output",""), str) and "i cannot" in e["output"].lower(),
    "hallucination_hint": lambda e: any(tag in (e.get("flags") or []) for tag in ["low_confidence","contradiction","unsupported_claim"]),
...
def check_thresholds(metrics: Dict[str, float], thresholds: Dict[str, float]) -> List[Dict[str, Any]]:
    alerts = []
    for k, v in (thresholds or {}).items():
        ...
        if k.endswith("_min"):
            if cur < v:
                alerts.append({"metric": k, "value": cur, "threshold": v, "op": "<", "status": "breach"})
        else:
            if cur > v:
                alerts.append({"metric": k, "value": cur, "threshold": v, "op": ">", "status": "breach"})
    return alerts
