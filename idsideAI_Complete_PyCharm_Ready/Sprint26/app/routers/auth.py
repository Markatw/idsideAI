...
def signup(payload: dict, request: Request):
    ...
    request.session["user"] = {
        k: v for k, v in USER_STORE[username].items() if k != "pw_hash"
    }
    _audit("signup", username)
    return {"ok": True, "user": request.session["user"]}
        ...
        out.append(
            {
                "username": name,
                "since": rec.get("since"),
                "last_refreshed": rec.get("last_refreshed"),
            }
        )
...
def bulk_purge_users(payload: dict, request: Request):
    ...
    for name in items:
        ...
        if not prof.get("is_deleted"):
            out.append({"username": name, "ok": False, "error": "not deleted"})
            continue
        try:
            del USER_STORE[name]
...
def export_audit_range(request: Request, payload: dict):
    ...
    do_zip = bool((payload or {}).get("zip"))
    try:
...
def export_audit_all(request: Request):
    ...
    return {
        "filename": "audit_full_export.zip",
        "bundle_b64": base64.b64encode(buf.getvalue()).decode("ascii"),
        "sha256": digest,
        "count": len(items),
    }
