...
@router.post("")
def create_graph():
    gid = str(uuid4()); _store[gid] = GraphModel()
    save_graph(gid, _store[gid]); return {"graph_id": gid, "graph": _store[gid]}

def _get(gid: str) -> GraphModel:
    g = _store.get(gid); 
    if not g: raise HTTPException(status_code=404, detail="graph not found")
    return g
...
def add_node(gid: str, node: NodeModel):
    ...
    g.nodes.append(node); save_graph(gid, g); 
    return g
...
def add_edge(gid: str, edge: EdgeModel):
    ...
    g.edges.append(edge); save_graph(gid, g); 
    return g
...
def del_edge(gid: str, idx: int):
    ...
    g.edges.pop(idx); save_graph(gid, g); 
    return g
...
class ExecRequest(BaseModel):
    start: str
    choices: Optional[List[str]] = None  # sequence of edge labels; '*' = auto if single choice
...
def execute(gid: str, req: ExecRequest):
    ...
    while True:
        ...
        opts = [e for e in g.edges if e.source == cur or (not e.directed and e.target == cur)]
        if not opts:
            break
        chosen = None
        ...
        cur = to; path.append(cur); idx += 1
        if idx >= len(choices):
            # stop if no more directives and branching exists
            more = [e for e in g.edges if (e.source == cur or (not e.directed and e.target == cur))]
            if len(more) != 1: break
...
def graph_summary(gid: str):
    ...
    ids = [nd.id for nd in g.nodes]; idset = set(ids)
    ...
    return {
        "graph_id": gid,
        "nodes": n, "edges": m,
        "directed_edges": dir_m, "undirected_edges": undir_m,
        "degree_min": dmin, "degree_max": dmax, "degree_avg": round(davg, 3),
...
def search_path(gid: str, req: SearchRequest):
    ...
    for lbl in req.labels:
        opts = [e for e in g.edges 
                if (e.source == cur or (not e.directed and e.target == cur)) 
                and (e.label or "") == lbl]
        if not opts:
            return {"status": "not_found", "at": cur, "wanted": lbl, "path": path, "steps": steps}
        chosen = opts[0]
        to = chosen.target if chosen.source == cur else chosen.source
        steps.append({"from": cur, "to": to, "label": lbl})
        cur = to; path.append(cur)
    return {"status": "ok", "path": path, "steps": steps}
...
def import_csv(gid: str, payload: CSVImport):
    ...
    for r in rdr:
        s = (r.get("source") or "").strip(); t = (r.get("target") or "").strip()
        if not s or not t: 
            continue
        lbl = r.get("label") or None
