...
def create_ws(request: Request, payload: dict):
    if not _is_admin(request): raise HTTPException(status_code=403, detail="forbidden")
    name = (payload or {}).get("name") or "workspace"
    ws_id = (payload or {}).get("id") or uuid.uuid4().hex[:12]
    WORKSPACES[ws_id] = {"id": ws_id, "name": name, "owner": "admin", "created_at": datetime.utcnow().isoformat(), "settings": {"title": name, "description": "", "plan": "free"}}
    _audit("ws_create", "admin"); _activity_add(ws_id, "ws_create", "admin"); ROLES.setdefault(ws_id, {})["admin"]="owner"; return WORKSPACES[ws_id]

@router.get("")
def list_ws(request: Request):
    if not _is_admin(request): raise HTTPException(status_code=403, detail="forbidden")
    _audit("ws_list", "admin"); return {"count": len(WORKSPACES), "items": list(WORKSPACES.values())}

@router.delete("/{ws_id}")
def delete_ws(ws_id: str, request: Request):
    user=_user(request) or "admin"
    if ws_id not in WORKSPACES: raise HTTPException(status_code=404, detail="workspace not found")
    _require(ws_id, user, {"owner"})
    ok = bool(WORKSPACES.pop(ws_id, None)); _audit("ws_delete", user); _activity_add(ws_id, "ws_delete", user)
    ROLES.pop(ws_id, None); MEMBERS.pop(ws_id, None); INVITES.pop(ws_id, None)
...
def invite_member(ws_id: str, request: Request, payload: dict):
    ...
    if not username: raise HTTPException(status_code=400, detail="missing username")
    INVITES.setdefault(ws_id, set()).add(username)
    _audit("ws_invite", "admin"); _activity_add(ws_id, "ws_invite", "admin", {"username": username})
...
def join_workspace(ws_id: str, request: Request):
    ...
    if ws_id not in WORKSPACES: raise HTTPException(status_code=404, detail="workspace not found")
    if user not in INVITES.get(ws_id, set()): raise HTTPException(status_code=403, detail="not invited")
    INVITES[ws_id].discard(user)
    MEMBERS.setdefault(ws_id, set()).add(user)
    _audit("ws_join", user); _activity_add(ws_id, "ws_join", user)
...
def leave_workspace(ws_id: str, request: Request):
    user = _user(request) or "guest"
    if ws_id not in WORKSPACES: raise HTTPException(status_code=404, detail="workspace not found")
    if user not in MEMBERS.get(ws_id, set()): raise HTTPException(status_code=404, detail="not a member")
    MEMBERS[ws_id].discard(user)
    _audit("ws_leave", user); _activity_add(ws_id, "ws_leave", user)
...
def set_role(ws_id: str, request: Request, payload: dict):
    ...
    if ws_id not in WORKSPACES: raise HTTPException(status_code=404, detail="workspace not found")
    tgt = (payload or {}).get("username"); role = (payload or {}).get("role")
    if role not in {"owner","admin","member"}: raise HTTPException(status_code=400, detail="invalid role")
    if role == "owner":
        _require(ws_id, actor, {"owner"})
    else:
        _require(ws_id, actor, {"owner","admin"})
    ROLES.setdefault(ws_id, {})[tgt] = role
    _audit("ws_role_set", actor); _activity_add(ws_id, "ws_role_set", actor, {"username": tgt, "role": role})
...
def switch_workspace(request: Request, payload: dict):
    ...
    if user not in MEMBERS.get(ws_id, set()) and _role(ws_id, user) not in {"owner","admin"}:
        raise HTTPException(status_code=403, detail="not a member")
    _ctx_set(request, ws_id)
    _audit("ws_switch", user); _activity_add(ws_id, "ws_switch", user); _presence_touch(ws_id, user)
...
def patch_settings(ws_id: str, request: Request, payload: dict):
    ...
    if ws_id not in WORKSPACES: raise HTTPException(status_code=404, detail="workspace not found")
    _require(ws_id, actor, {"owner","admin"})
    st = WORKSPACES[ws_id].setdefault("settings", {"title":"", "description":"", "plan":"free"})
    for k in ("title","description","plan"):
        if k in (payload or {}): st[k] = payload[k]
    _audit("ws_settings_update", actor); _activity_add(ws_id, "ws_settings_update", actor)
    return st


def _activity_add(ws_id: str, event: str, actor: str, extra: dict | None = None):
    ACTIVITY.setdefault(ws_id, []).append({
        "ts": datetime.utcnow().isoformat(), "event": event, "actor": actor, "extra": extra or {}
    })
...
def export_audit(ws_id: str, request: Request):
    ...
    ws = WORKSPACES[ws_id]
    return {
        "workspace": {"id": ws_id, "name": ws.get("name"), "created_at": ws.get("created_at")},
...
def post_chat(ws_id: str, request: Request, payload: dict):
    actor = _user(request) or "guest"
    if ws_id not in WORKSPACES: raise HTTPException(status_code=404, detail="workspace not found")
    if actor not in MEMBERS.get(ws_id, set()) and _role(ws_id, actor) not in {"owner","admin"}:
        raise HTTPException(status_code=403, detail="not allowed")
    text = (payload or {}).get("text")
    if not text or not isinstance(text, str): raise HTTPException(status_code=400, detail="missing text")
    _chat_add(ws_id, actor, text); _presence_touch(ws_id, actor); _activity_add(ws_id, "chat_post", actor, {"chars": len(text)})
...
def _doc_make(ws_id: str, doc_id: str, title: str, content: str, owner: str):
    DOCS.setdefault(ws_id, {})[doc_id] = {
        "id": doc_id, "title": title, "content": content, "owner": owner,
...
def create_doc(ws_id: str, request: Request, payload: dict):
    ...
    if actor not in MEMBERS.get(ws_id, set()) and _role(ws_id, actor) not in {"owner","admin"}:
        raise HTTPException(status_code=403, detail="not allowed")
    doc_id = (payload or {}).get("id"); title = (payload or {}).get("title") or ""; content = (payload or {}).get("content") or ""
    if not doc_id or not isinstance(doc_id, str): raise HTTPException(status_code=400, detail="missing id")
    if doc_id in (DOCS.get(ws_id, {}) or {}): raise HTTPException(status_code=409, detail="doc exists")
...
def read_doc(ws_id: str, doc_id: str, request: Request):
    user = _user(request) or "guest"
    if ws_id not in WORKSPACES or ws_id not in DOCS: raise HTTPException(status_code=404, detail="workspace not found")
    if user not in MEMBERS.get(ws_id, set()) and _role(ws_id, user) not in {"owner","admin"}:
        raise HTTPException(status_code=403, detail="not allowed")
...
def update_doc(ws_id: str, doc_id: str, request: Request, payload: dict):
    ...
    if actor != doc.get("owner") and _role(ws_id, actor) not in {"owner","admin"}:
        raise HTTPException(status_code=403, detail="not allowed")
    if "title" in (payload or {}): doc["title"] = payload["title"]
    if "content" in (payload or {}): doc["content"] = payload["content"]
