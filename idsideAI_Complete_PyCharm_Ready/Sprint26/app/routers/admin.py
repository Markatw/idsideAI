...
def import_settings(request: Request, payload: dict):
    if not _is_admin(request):
        raise HTTPException(status_code=403, detail="forbidden")
    data = payload or {}
    blob_b64 = data.get("bundle_b64")
    expect_hash = data.get("sha256")
...
def rollback_settings(request: Request, payload: dict):
    ...
    SETTINGS_STORE.clear()
    SETTINGS_STORE.update(dict(snap.get("settings", {})))
...
def compliance_export(request: Request):
    ...
    return {
        "filename": "settings_compliance.zip",
        "bundle_b64": base64.b64encode(zfinal.getvalue()).decode(),
        "sha256": digest,
        "history_count": len(SETTINGS_HISTORY),
    }
