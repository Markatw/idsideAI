...
def saml_metadata_xml(entity_id: Annotated[str, Query(""), acs_url: Annotated[str, Query("")) -> str:
    return saml_metadata(entity_id, acs_url)


@router.get("/oidc/.well-known/openid-configuration")
def oidc_config(issuer: Annotated[str, Query(""), base_url: Annotated[str, Query("")) -> Dict[str, Any]:
    return oidc_discovery(issuer, base_url)


@router.post("/test")
def sso_test(provider: Annotated[str, Body("oidc"), email: Annotated[str, Body("")) -> Dict[str, Any]:
