import os, json, sqlite3, time, pathlib
_DB = os.environ.get("GRAPH_DB_PATH", "data/graphs.db")
def _conn():
    pathlib.Path(_DB).parent.mkdir(parents=True, exist_ok=True)
    con = sqlite3.connect(_DB)
    con.execute("CREATE TABLE IF NOT EXISTS graphs (id TEXT PRIMARY KEY, graph_json TEXT NOT NULL, updated_at INTEGER NOT NULL)")
    return con
def save_graph(gid: str, graph_model):
    js = json.dumps(graph_model.dict(), separators=(',',':'))
    with _conn() as c:
        c.execute("REPLACE INTO graphs(id,graph_json,updated_at) VALUES (?,?,?)",(gid, js, int(time.time())))
